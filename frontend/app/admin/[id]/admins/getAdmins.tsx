...
export default function getAdmins() {
    ...
    async function fetchData() {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/admin/getAdmins`, { withCredentials: true });
            ...
            {admins.map((item) => (
                <div key={item.id} className="relative w-full min-w-0 bg-base-100 shadow-md rounded-lg p-4 flex flex-col md:flex-row gap-4" >
                    <div className="flex flex-col gap-3 shrink-0">
                        <img src={`${process.env.NEXT_PUBLIC_API_URL}/admin/getAdminPicByID/${item.id}`} alt={item.username} className="w-48 object-contain rounded" />
                    </div>
